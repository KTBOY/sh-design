...
<p>
  ...
  <a href="https://ktboy.github.io/sh-design/components/lazy-image"><b>🧩 在线组件预览</b></a> ·
...
- 🧩 **业务组件优先** — 不是 Button/Input 这类基础组件，更聚焦懒加载图片、无缝滚动等**真实业务里反复要写**的功能型组件，开箱即用。
...
**新增组件**：参考 `packages/sh-design/src/components/lazy-image/`（组件目录约定）→ 在 `src/components/index.ts` 导出 → 在 `docs/components/` 增加文档页。
